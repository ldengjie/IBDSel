...
*/
	for( int i=0 ; i<GlobalVar::NumADs ; i++ )
	{
		...
		histName+=i+1;
		histName+="NumOfAll";
		NumOfAll[i] = new TH1F(histName,"num of all triggers more than 0.7MeV",liveTimeSvc->nBins(),liveTimeSvc->startTime().AsDouble(),liveTimeSvc->endTime().AsDouble());
		ntupleSvc()->attach("FILE1/lidj",NumOfAll[i]);
		histName="AD";
		histName+=i+1;
		histName+="NumOfAll0";
		NumOfAll0[i] = new TH1F(histName,"num of all triggers more than 0.7MeV",liveTimeSvc->nBins(),liveTimeSvc->startTime().AsDouble(),liveTimeSvc->endTime().AsDouble());
...
*/
	...
	if( CurEvent->isMuon())
	{
        ...
        for( unsigned int i=0 ; i<calib->nHitsAD ; i++ )
        {
            ...
            if( calib->hitCountAD.at(i)==0 )
            {
                ...
                chargeADtotalAfterCor+=calib->chargeAD.at(i)/SaturationCoefficient;
        ...
        AdMuonEnergy=EnergyAfterSatAndHetCor;
        */
    ...
    if( CurEvent->energy()>6 && CurEvent->energy()<12.0 )
    {
        isNeulike=true;
    } 
...
{
    //std::cout<<"num_total  : "<<num_total<<endl;
    //std::cout<<"num_temp  : "<<num_temp<<endl;
    //std::cout<<"num_temp_2  : "<<num_temp_2<<endl;
    //std::cout<<"num_temp_N0  : "<<num_temp_N0<<endl;
    PhyEvent* CurEvent=0;
    for( int i=1 ; i<5 ; i++ )
    {
        dump(i,1,CurEvent);
...
bool IBDSel::FillIbd(vector<PhyEvent*> EvtGroup,int tag)
{
	...
	if( !(((EvtGroup[1]->m_trigTime-EvtGroup[0]->m_trigTime)>=IbdTimeIntervalMin)&&((EvtGroup[1]->m_trigTime-EvtGroup[0]->m_trigTime)<=IbdTimeIntervalMax)) )
	{
		return true;
	}

	//if( !((EvtGroup[0]->energy()>promptELow) && (EvtGroup[0]->energy()<promptEHigh)&&(EvtGroup[1]->energy()>delayedELow)&&(EvtGroup[1]->energy()<delayedEHigh)) )
	if( !((EvtGroup[0]->energy()>=promptELow) && (EvtGroup[0]->energy()<=promptEHigh4Fn)&&(EvtGroup[1]->energy()>=delayedELow)&&(EvtGroup[1]->energy()<=delayedEHigh)) )
	{
		return true;
	}
    ...
    IsLi=7;
	if( (EvtGroup[0]->energy()<promptEHigh)&&(EvtGroup[0]->energy()>3.5)&&((EvtGroup[1]->m_trigTime-EvtGroup[0]->m_trigTime)<1.e-4) )
	{
		...
		if( tag==2 )
		{
			if( delayedT2Muon[4]>=DelayedTrigTime2RpcMuon4Li && delayedT2Muon[8]>=DelayedTrigTime2AdMuon4Li && delayedT2Muon[9]>=DelayedTrigTime2AdShowerMuon4Li)
			{
	    		IsLi=2;
			}else
			{
	...
	//CalTime2Muon(EvtGroup[1]);


	if( !((delayedT2Muon[0]>=DelayedTrigTime2AdMuon)&&(delayedT2Muon[2]>=DelayedTrigTime2IWpMuon)&&(delayedT2Muon[1]>=DelayedTrigTime2AdShowerMuon)&&(delayedT2Muon[3]>=DelayedTrigTime2OWpMuon)) )
	{
        if( IsLi==7 )
        {
            return true;
	...
	}else
    {
		if( tag==2 )
		{
			if( delayedT2Muon[4]>=DelayedTrigTime2RpcMuon && delayedT2Muon[8]>=DelayedTrigTime2AdMuon && delayedT2Muon[9]>=DelayedTrigTime2AdShowerMuon)
			{
				IsIbd=2;
			}else
			{
...
/*
	if( !(delayedT2Muon[4]>=DelayedTrigTime2RpcMuon) )
	{
		IsIbd=0;
	}
	
	if( (VetoedBuf[EvtGroup[1]->m_det-1][0]==1)||(VetoedBuf[EvtGroup[1]->m_det-1][1]==1) )
	{
		IsIbd=0;
	}else
	{
